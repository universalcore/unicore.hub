...
class AppApiTestCase(DBTestCase):
    ...
    def test_view(self):
        resp = self.app.get(
            '/apps/%s' % self.user_app.id,
            headers=self.get_basic_auth_header(self.user_app.slug, 'password'))
    ...
    def test_edit(self):
        ...
        resp = self.app.put_json(
            ...
            params={'title': 'foo2'})
        self.db.expire_all()
        new_user_app = self.db.query(App).get(self.user_app.id)
        self.assertEqual(resp.status_int, 200)
        ...
        resp = self.app.put_json(
            '/apps/%s' % self.user_app.id,
            headers=self.get_basic_auth_header(self.manager_app.slug, 'password'),
    ...
    def test_create(self):
        ...
        new_app = self.db.query(App).get(resp.json_body['id'])
    ...
    def test_reset_password(self):
        resp = self.app.put(
            '/apps/%s/reset_password' % self.user_app.id,
            headers=self.get_basic_auth_header(self.user_app.slug, 'password'))
        self.db.expire_all()
        new_user_app = self.db.query(App).get(self.user_app.id)
        self.assertEqual(resp.status_int, 200)
